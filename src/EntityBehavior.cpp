...
void EntityBehavior::findTarget() {
	...
	// Host only: a remote net player closer than the host's hero is the better target
	targeting_net_player = false;
	if (!e->stats.hero_ally) {
		for (size_t t = 0; t < entitym->net_targets.size(); ++t) {
			StatBlock *nt = entitym->net_targets[t].stats;
			if (!nt->alive)
				continue;
			float nt_dist = Utils::calcDist(e->stats.pos, nt->pos);
			if (!target_stats || nt_dist < target_dist) {
				target_stats = nt;
				target_dist = nt_dist;
				targeting_net_player = true;
			}
		}
	}
	...
	// check line-of-sight
	if (target_stats && target_dist < e->stats.threat_range && (pc->stats.alive || targeting_net_player))
	...
	if (!e->stats.alive || (!pc->stats.alive && !targeting_net_player) || (target_stats && !target_stats->alive))
		e->stats.in_combat = false;
...
void EntityBehavior::checkMoveStateMove() {
	...
	if ((pc->stats.alive || targeting_net_player) && ((target_dist < e->stats.melee_range && !fleeing) || (move_to_safe_dist && target_dist >= e->stats.flee_range) || stop_fleeing || ally_targeting_hero)) {
		if (stop_fleeing) {
