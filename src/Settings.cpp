...
/**
 * Guess a Flare language code (see mods/default/engine/languages.txt) from
 * the OS locale, so first-time launch defaults to the player's own language
 * instead of always "en". Only ever used as the *default* before the
 * language dialog / config menu offers an explicit choice, so an unknown
 * or unsupported guess is harmless -- it just falls back to English text.
 */
std::string Settings::detectSystemLanguage() {
	...
	if (GetUserDefaultLocaleName(buf, LOCALE_NAME_MAX_LENGTH) > 0) {
		...
		locale_str = narrow;
		for (size_t i = 0; i < locale_str.size(); i++) {
			if (locale_str[i] == '-') locale_str[i] = '_';
		}
	}
#else
	const char *env_locale = getenv("LANGUAGE");
	if (!env_locale || env_locale[0] == '\0') env_locale = getenv("LC_ALL");
	if (!env_locale || env_locale[0] == '\0') env_locale = getenv("LC_MESSAGES");
	if (!env_locale || env_locale[0] == '\0') env_locale = getenv("LANG");
	if (env_locale) locale_str = env_locale;

	// LANGUAGE can be a colon-separated priority list; just take the first entry
	size_t colon = locale_str.find(':');
	if (colon != std::string::npos) locale_str = locale_str.substr(0, colon);
#endif

	// strip encoding/modifier, e.g. "pt_BR.UTF-8" -> "pt_BR", "de_DE@euro" -> "de_DE"
	size_t cut = locale_str.find_first_of(".@");
	if (cut != std::string::npos) locale_str = locale_str.substr(0, cut);

	if (locale_str.empty() || locale_str == "C" || locale_str == "POSIX") return "en";

	// languages.txt only carries a region suffix for a handful of codes;
	// everything else is just the 2-letter language part.
	static const char *region_codes[] = { "pt_BR", "hi_IN", "zh_TW", "th_TH" };
	for (size_t i = 0; i < sizeof(region_codes) / sizeof(region_codes[0]); i++) {
		if (locale_str == region_codes[i]) return locale_str;
	}

	size_t underscore = locale_str.find('_');
	if (underscore != std::string::npos) locale_str = locale_str.substr(0, underscore);
