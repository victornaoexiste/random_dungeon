...
void Utils::createLogFile() {
	// Distinct filename per net role so two instances on the same machine
	// (host + client, testing multiplayer) don't stomp on each other's log --
	// each was truncating/interleaving into the same ~/.config/flare/flare_log.txt
	// otherwise, making it unreliable to actually debug from. Only affects
	// --net-host/--net-join launches; plain single-player keeps the old name.
	std::string suffix;
	if (!settings->net_host_port.empty())
		suffix = "_host";
	else if (!settings->net_join_target.empty())
		suffix = "_client";

	LOG_PATH = settings->path_conf + "/flare_log" + suffix + ".txt";
