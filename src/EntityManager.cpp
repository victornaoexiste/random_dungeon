...
void EntityManager::handleNewMap () {
	...
	while (!mapr->enemies.empty()) {
		...
		// A net client doesn't run its own copy of the horde -- it mirrors
...
void EntityManager::logic() {
	...
	for (it = entities.begin(); it != entities.end(); ++it) {
		// A net client's proxy enemies (see StatBlock::net_proxy) have their
		// position/hp/animation forced from the network every tick in
		// GameStatePlay::syncRemoteEnemies -- running the normal AI/movement
		// logic here would just fight that, so only advance the animation
		// frame, the same way GameSlotPreview::logic() does for remote
		// players' visuals.
		if ((*it)->stats.net_proxy) {
			if ((*it)->activeAnimation)
				(*it)->activeAnimation->advanceFrame();
			for (size_t i = 0; i < (*it)->anims.size(); i++) {
				if ((*it)->anims[i])
					(*it)->anims[i]->advanceFrame();
			}
			continue;
		}
