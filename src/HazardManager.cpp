...
void HazardManager::logic() {
	...
	for (size_t i=h.size(); i>0; i--) {
		...
		// handle collisions (skipped for a cosmetic-only remote-power visual --
		// see Hazard::cosmetic_only -- it still moves/animates/expires above)
		if (hazard->isDangerousNow() && !hazard->cosmetic_only) {
			...
			// enemy/neutral hazards can also hurt remote net players (host only)
			if (hazard->source_type != Power::SOURCE_TYPE_HERO && hazard->source_type != Power::SOURCE_TYPE_ALLY && hazard->active) {
				for (size_t t = 0; t < entitym->net_targets.size(); ++t) {
					const EntityManager::NetTarget& nt = entitym->net_targets[t];
					if (!nt.stats->alive || !hazard->active)
						continue;
					if (std::find(hazard->net_players_hit.begin(), hazard->net_players_hit.end(), nt.id) != hazard->net_players_hit.end())
						continue;
					if (!Utils::isWithinRadius(hazard->pos, hazard->power->radius, nt.stats->pos))
						continue;

					hazard->net_players_hit.push_back(nt.id);
					NetPlayerHit hit;
					hit.player_id = nt.id;
