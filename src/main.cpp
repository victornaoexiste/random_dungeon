...
int main(int argc, char *argv[]) {
	...
	for (int i = 1 ; i < argc; i++) {
		...
		else if (arg == "net-server") {
			cmd_line_args.net_server_port = parseArgValue(arg_full);
			if (cmd_line_args.net_server_port.empty())
				cmd_line_args.net_server_port = "4650";
		}
		else if (arg == "net-client") {
			cmd_line_args.net_client_target = parseArgValue(arg_full);
		}
		else if (arg == "net-host") {
			settings->net_host_port = parseArgValue(arg_full);
			if (settings->net_host_port.empty())
				settings->net_host_port = "4650";
		}
		else if (arg == "net-join") {
			settings->net_join_target = parseArgValue(arg_full);
		}
		else if (arg == "help") {
...
--net-join=HOST[:PORT]   Plays normally, connecting to a host's game server at HOST:PORT.");
	...
	// Headless network transport smoke test. Deliberately kept outside of
	...
	if (!done && (!cmd_line_args.net_server_port.empty() || !cmd_line_args.net_client_target.empty())) {
		...
		if (!cmd_line_args.net_server_port.empty()) {
			uint16_t port = static_cast<uint16_t>(atoi(cmd_line_args.net_server_port.c_str()));
			if (net.startServer(port))
				net.runServerTestLoop();
		}
		else {
			std::string host = cmd_line_args.net_client_target;
			uint16_t port = 4650;
			...
			if (net.connectToServer(host, port, 5000))
				net.runClientTestLoop();
		}
...
soft_reset:
	if (!done) {
		...
		if (!settings->net_host_port.empty() || !settings->net_join_target.empty()) {
			...
			if (!settings->net_host_port.empty()) {
				...
				netmgr->connectToServer(host, port, 5000);
			}
		}
